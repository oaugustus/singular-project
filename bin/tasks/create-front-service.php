...
$console
    ...
    ->setDescription('Cria um novo servico de front-end')
    ->setCode(function (InputInterface $input, OutputInterface $output) use ($app) {
        ...
        $destDir = $app['web_dir']."/src/".$dest."/services/";

        if ($fs->exists($destDir)){

            $tpl = file_get_contents(__DIR__."/templates/FrontService.tpl");
            $tpl = str_replace('$name', $name, $tpl);
            $tpl = str_replace('$module', $module, $tpl);

            $fs->dumpFile($destDir.$name.".js", $tpl);

            $output->writeln("<info>O servico ".$input->getArgument("name")." foi criado com sucesso!</info>");
